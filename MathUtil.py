...
def indicator_macd(close, periods, gradient=False, sign=False):
...
def indicator_rsi(close, period, gradient=False, sign=False):
    rsi = RSIIndicator(close, period)._rsi
    result = rsi / 100.0 - 0.5
    return apply_transforms(result, gradient, sign)

def indicator_sma(close, period, gradient=False, sign=False):
    result = close.rolling(period).mean()
    return apply_transforms(result, gradient, sign)

def indicator_vwap(high, low, close, volume, period, gradient=False, sign=False):
    vwap = VolumeWeightedAveragePrice(high, low, close, volume, period).vwap
    result = fast_sigmoid((vwap - close) / close * 100.0)
    return apply_transforms(result, gradient, sign)

def indicator_cci(high, low, close, period, gradient=False, sign=False):
    cci = CCIIndicator(high, low, close, period)._cci
    result = fast_sigmoid(cci / 100.0)
    return apply_transforms(result, gradient, sign)

def indicator_atr(high, low, close, period, gradient=False, sign=False):
    result = AverageTrueRange(high, low, close, window=period)._atr
    return apply_transforms(result, gradient, sign)

def indicator_premier_rsi(high, low, close, period, stochlen=8, smoothlen=25, gradient=False, sign=False):
...
def indicator_squeeze_momentum(high, low, close, bbPeriod=20, bbMult=2, kcPeriod=20, kcMult=1.5, useTrueRange=True, gradient=False, sign=False):
...
def indicator_test(close, period):
    return np.linspace(1, 1, num=len(close))